...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const errorMessage = err.response?.data?.error || err.response?.data?.message || err.message || 'Failed to send reset code. Please try again.';
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-950 px-6 py-12">
      <div className="w-full max-w-md p-10 bg-slate-900 border border-slate-850 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8 relative">
          ...
          <p className="text-slate-400 text-sm mt-2 font-medium text-center leading-relaxed">Enter your email and we'll send you an OTP to reset your password.</p>
        </div>
